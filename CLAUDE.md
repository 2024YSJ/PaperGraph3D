...
This repo (`PaperGraph3D`) is mid build-out. Features 001, 002, and 003 are implemented: `src/models/{subscription,paper,settings}.ts` hold the core data structures (types plus runtime validators, including the core `Paper.embedding`/`embeddingModel`/`embeddingSource` fields); `src/collection/` holds the 002 subscription-based collection pipeline (arXiv + Semantic Scholar clients, dedupe, enrichment, promotion, scheduler); and `src/persistence/` holds the 003 paper-note persistence layer (JSON record + Markdown note pairing). `src/main.ts` is now real plugin code that wires the 002 collection pipeline into 003 persistence (commit `d0d4002`), not the stock Obsidian sample — full lifecycle assembly is still owned by the future 008 feature. `src/settings.ts` holds the settings interface, defaults, and settings tab. `manifest.json` declares `id: "paper-graph-3d"` and, per the content-similarity-layout decision, `isDesktopOnly: true` (the project is deliberately desktop-only; see the constitution). Feature 005 (manual-paper-refresh) is the active feature — its spec is drafted under `specs/005-manual-paper-refresh/` (spec + requirements checklist; no plan yet).
...
Feature specs under `specs/`: `001-core-data-models/` (implemented under `src/models/`), `002-subscription-paper-collection/` (implemented under `src/collection/`), `003-paper-note-persistence/` (implemented under `src/persistence/`), and `005-manual-paper-refresh/` (spec drafted; not yet planned or implemented). Early drafts of every feature (001–008, including 004 summarization, 006 graph-data conversion, 007 3D visualization, 008 plugin assembly) live under `specs-input/`; the whole set threads a content-similarity x,y graph layout (year fixed to the z axis) driven by the core `Paper.embedding`.
