...
Feature specs under `specs/`: `001-core-data-models/`, `002-subscription-paper-collection/`, `003-paper-note-persistence/`, and `005-manual-paper-refresh/` — all four specified, planned, implemented (`src/models`, `src/collection`, `src/persistence`, `src/refresh`), and verified under `specs-test/`. The content-embedding thread (bundled baseline / local-transformer / LLM three-provider selector, FR-044/FR-045/FR-046) is folded into 002's spec + tasks (Phase 9). Drafts for 004 summarization, 006 graph-data conversion, 007 3D visualization, and 008 plugin assembly live under `specs-input/`; the whole set threads a content-similarity x,y graph layout (year fixed to the z axis) driven by the core `Paper.embedding`, produced by whichever embedding provider is selected.
