...
- **FR-010**: ~~The LLM API as a selectable embedding provider.~~ **[Withdrawn 2026-07-16]** — the LLM embedding provider is retired (constitution v1.3.0). Embeddings now come from one fixed on-device model, so this feature owns no embedding path, no embedding credential, and no embedding toggle. Kept numbered, not deleted, so existing references stay resolvable.
- **FR-011**: ~~The summarization toggle and the LLM-embedding toggle MUST be independent.~~ **[Withdrawn 2026-07-16]** — the LLM embedding provider is retired (constitution v1.3.0). Embeddings now come from one fixed on-device model, so this feature owns no embedding path, no embedding credential, and no embedding toggle. Kept numbered, not deleted, so existing references stay resolvable. There is only one toggle left to be independent of.
- **FR-012**: ~~An LLM embedding failure MUST fall back to the retained local baseline.~~ **[Withdrawn 2026-07-16]** — the LLM embedding provider is retired (constitution v1.3.0). Embeddings now come from one fixed on-device model, so this feature owns no embedding path, no embedding credential, and no embedding toggle. Kept numbered, not deleted, so existing references stay resolvable. The equivalent rule for the on-device model lives in 002 FR-045.
- **FR-013**: Before the user can enable this feature, the settings UI MUST disclose what data is sent to the configured provider (title and abstract only — never the full record, references, or any other paper's data), that the credential is stored in plaintext in the plugin's settings data, and that the feature is off until explicitly enabled (constitution Principle IV). Summarization is the only external data path this feature owns: embedding is on-device and sends nothing (002 FR-045/FR-046).
...
- **Summarization Settings**: The on/off flag (from 001) plus the chosen provider and credentials, which are extension fields (001 FR-016) defined by this feature and surfaced in the summarization section of the settings screen (008). Credentials are sensitive and handled per the project's transparent-use policy. This feature's scope is **summary text only** — it owns exactly one credential, for summarization. **[Amended 2026-07-16]** It previously also carried an independent embedding-upgrade toggle and its own embedding credential, widening its scope to "LLM-based paper enrichment"; that is withdrawn (FR-010/FR-011/FR-012, constitution v1.3.0). Embeddings come from one fixed on-device model owned by 002 — there is no embedding provider to select, no embedding key to store, and nothing for this feature to toggle.
