...
- **FR-002**: The JSON record MUST be the plugin's source of truth; the Markdown note's plugin-managed region MUST mirror the record for the user to read. The managed region has **two parts**: (a) the note's **YAML frontmatter**, holding the structured shared fields — `title`, `authors`, `publicationYear`, `citationCount`, `readState`, and `pg3d_sourceId`; and (b) a **delimited managed body block** immediately after the frontmatter, holding the rendered prose — the paper's `abstract`, replaced by the generated `summary` when 004 is enabled, plus the `futureDirections` text for qualifying papers. Raw `references` sourceId arrays, `schemaVersion`, and timestamps live only in the JSON record. Everything outside the frontmatter and the managed body block is the user's free-form body. The FR-002 consistency invariant is defined over the frontmatter shared fields plus the managed body block's rendered content. (The exact delimiter for the managed body block — e.g. HTML-comment markers — is a presentation detail settled in planning, alongside OQ-4.)
