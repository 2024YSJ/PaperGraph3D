...
**Rationale**: This feature is the plugin's first that performs real vault I/O, but almost all of its hard requirements are *logic* (field-scoped merge FR-004, tombstoned two-phase delete FR-021, JSON-authoritative reconciliation FR-013, injective filenames FR-007, the FR-015 read API). With no test runner in the repo (`CLAUDE.md`), the only way to verify that logic offline — via the `esbuild`+`node` `quickstart.md` pattern already established by 001 — is to keep it free of the Obsidian host. A thin port confines every side effect and the only Obsidian dependency to one adapter file (`filestore-obsidian.ts`), satisfying constitution Principle VI (single-responsibility, testable modules) and Principle IV (I/O confined and auditable).
...
**Decision**: A note is parsed into exactly three regions in order: **(1) YAML frontmatter** (`---` fenced at file top) holding the mirrored shared subset (`title`, `authors`, `publicationYear`, `citationCount`, `readState`, `pg3d_sourceId`, plus a `url` derived from `sourceId` at render time — not stored); **(2) a managed body block** delimited by HTML comment markers `<!-- pg3d:begin -->` … `<!-- pg3d:end -->` holding rendered prose (abstract, or 004's summary + future-directions); **(3) the user's free-form body** — everything after the managed block. Writes rewrite regions (1) and (2) wholesale and never touch (3). Frontmatter is produced/parsed by a minimal internal serializer for this fixed, plugin-owned schema (flat scalars + one `authors` block sequence), keeping `note.ts` Obsidian-free and offline-testable per §1/§10; the output is standard YAML Obsidian's metadata cache still reads.
...
## 3. Filename derivation — human-readable title stem under `<YYYY>/<MM>/<DD>/` folders (amended 2026-07-13)

**Decision (amended, FR-007 Session 2026-07-13)**: `noteStem(paper)` = `<YYYY>/<MM>/<DD>/` + a normalized/sanitized/length-capped **title** + ` (` + the provider-local id + `)`, e.g. `2024/03/15/Attention Is All You Need (2401.12345)`. The date folder's **year** is `publicationYear` (always present), the **month** and **day** are from the optional `publicationDate` (001 FR-023); when only a bare year is known (no full date) the folder falls back to `<YYYY>/unknown`. The load scan **recurses** through the date subfolders (FileStore's recursive `list()`) and pairs by content id; writes create the parent folders (`mkdir -p`). The full relative stem (folder + name) is fixed at creation and reused on updates, so a refresh never re-folders. Normalization: NFC-normalize, replace filesystem-illegal/reserved characters (`\ / : * ? " < > |`, control chars) and internal whitespace (incl. arXiv's line-wrapped titles) with a single space, collapse/trim, cap at 100 chars. The parenthesized provider-local id makes the stem injective (two papers differ in id even at an identical truncated title); a deterministic `-2` disambiguator handles any residual clash (index-backed `sourceId ↔ stem` map). When the title sanitizes to empty, fall back to the original sourceId sanitization `fileStem(sourceId)` = `provider` prefix + `_` + sanitized local part (e.g. `arxiv:2401.12345` → `arxiv_2401.12345`). The `.json` and `.md` share the stem, fixed at creation and reused on updates (a later title revision does not rename). The filename is **not** authoritative — content-level `sourceId` (FR-009) is.

**Rationale**: The earlier sourceId-only stem surfaced an opaque arXiv id (`arxiv_2401.12345`) as the note's name in Obsidian's file explorer, which is unfriendly for browsing a personal library. A title-based name is what a reader expects; the id suffix preserves the injectivity and traceability the sourceId stem provided, and keying pairing on content (FR-009) keeps it rename-safe.

**Alternatives considered**:
- *Keep the sourceId stem, show the title via Obsidian aliases / a display plugin*: rejected — the file explorer still shows the filename without an external plugin; the user asked for the filename itself to read as the title.
- *Keep the original sourceId-only stem (`arxiv_2401.12345`)*: the previous decision; superseded because it surfaces an opaque id as the note name. Retained only as the empty-title fallback.
...
- *Percent-encode the colon*: rejected — `%3A` is legal but ugly and still needs collision handling for other characters; the readable form is friendlier.
