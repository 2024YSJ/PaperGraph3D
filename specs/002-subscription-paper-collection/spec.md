...
- If the system clock moves backward past a subscription's last-checked time, that check searches an empty window rather than sending a provider an invalid (reversed) date range, and still advances `lastCheckedAt`.
...
- **FR-021**: Every check MUST read the current `summarizationEnabled` setting and Semantic Scholar API key at the moment it runs, never a value captured once when the plugin loaded. If summarization is turned off while a specific paper's summary generation is already in flight, this feature MUST discard that in-flight result upon completion and fall back to the abstract (004's own fallback), rather than persisting a summary generated after the setting was turned off.
- **FR-022**: When more than one subscription becomes due in the same scheduled tick or catch-up pass, they MUST be checked one at a time, sequentially — never concurrently.
...
- **FR-024**: If the current time is earlier than a subscription's last-checked time (e.g., a system clock moved backward), the computed search window MUST NOT be sent to a provider as an inverted range; the check MUST instead use an empty window and still advance `lastCheckedAt` to the current time.
