...
**Backfill (FR-033–040) re-check**: the historical-backfill addition does not disturb any principle. It is even more clearly opt-in than forward collection (Principle IV — a manual, per-subscription, per-run user trigger, never automatic on registration); its network calls stay confined to the same two provider clients (Principle I/IV — no new external surface); and its runner is driven entirely by the existing `registerInterval` tick / `backfillNow` handle with no raw timer of its own (Principle II — no new teardown obligation). The `backfill.ts` runner is a new single-responsibility module (Principle VI), and its two `Subscription` fields are additive-only 001 extensions (Additional Constraints — no 001 field removed or redefined).
...
├── models/                       # existing (001) — Paper/PaperCandidate read-only; subscription.ts gains two optional backfill fields coveredFrom/backfillState (FR-033–040, FR-016-style additive extension); settings.ts gains one optional field semanticScholarApiKey (FR-020, FR-016 extension)
└── collection/                   # NEW — this feature's entire footprint
    ├── subscriptionStore.ts       # Subscription[] CRUD (register/list/delete/enable/setInterval) + backfill ops (requestBackfill/recordBackfillProgress/recordFirstCoverage) + persistence read/write hook; the settings-screen UI (008) calls this, it builds no UI itself
    ...
    ├── scheduler.ts               # ONE 15-min registerInterval tick (not one timer per subscription — research.md Decision 4) that checks each due subscription + single catch-up-on-load pass + immediate checkNow on new registration (also serving FR-032's on-demand check) + resuming any active backfill; owns lastCheckedAt advancement; returns { checkNow, backfillNow }
    ├── pipeline.ts                # per-paper orchestration: enrich -> (hook) summarize -> hand off to (hook) persist; also owns the sequential, non-blocking batch loop (runCollectionPass) over a discovered batch; runSubscriptionCheck reused unchanged by backfill (only the window differs)
    └── backfill.ts                # NEW — backfill runner: resumes an active backfillState subscription pass-by-pass over [cursor, coveredFrom) (oldest-first), reusing runSubscriptionCheck + the shared in-flight guard; advances/persists cursor via recordBackfillProgress; never touches lastCheckedAt (FR-033–040)
```

**Structure Decision**: Single project (no frontend/backend split). All new code lives under a new `src/collection/` directory, one file per responsibility, matching the constitution's single-responsibility module rule and mirroring 001's `src/models/` precedent. `src/main.ts` gains only the wiring needed to start the scheduler through `registerInterval` in `onload` (no `onunload` teardown code is needed — Obsidian auto-clears a `registerInterval`-registered tick); the other existing files modified are `src/models/settings.ts`, which gains a single optional `semanticScholarApiKey?: string` field on `PluginSettings`, and `src/models/subscription.ts`, which gains two optional backfill fields (`coveredFrom?`, `backfillState?`) with `isValidSubscription` widened to accept them (both FR-016-permitted additive extensions, FR-020 / FR-033–040 — no 001 field removed or redefined). `pipeline.ts` calls into 003 (persistence) and 004 (summarization) through narrow function-call hooks defined in this feature's contract, not through direct imports of their internals, so this feature can be implemented and tested (with stub hooks) before 003/004 exist. `subscriptionStore.ts` owns subscription CRUD and persistence (per 008's clarification that 008 builds only the settings-screen UI and calls into the feature that owns the underlying data); this feature's own "management" User Story is therefore satisfied by these functions alone, with no UI, exactly like 001's data-only-no-UI precedent.
