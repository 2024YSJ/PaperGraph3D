...
- [ ] T006 [P] [US4] Implement `queryArxiv(subscription, window)` using `requestUrl` against the arXiv API, returning raw Atom `<entry>` DOM nodes plus a `truncated` flag, in `src/collection/arxivClient.ts` (per `contracts/collection-pipeline.md` § arxivClient.ts) (FR-008, FR-014)
- [ ] T007 [P] [US4] Implement `fetchSemanticScholarPaper(identity)` using `requestUrl` (arXiv-ID lookup, falling back to title+author search), returning the raw JSON response or `undefined`, in `src/collection/semanticScholarClient.ts` (FR-008)
...
- [ ] T010 [US4] Implement `enrichFromSemanticScholar(candidate): Promise<EnrichmentOutcome>` — arXiv-ID match first, title+first-author fallback, terminal-absence vs. bounded-transient-failure (max 3 attempts) classification — in `src/collection/enrichment.ts` (per `contracts/collection-pipeline.md` § enrichment.ts) (FR-016, FR-018; depends on T007, T009)
