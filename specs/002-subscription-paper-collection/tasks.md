...
- [ ] T008 [P] [US4] Implement `parseArxivAtom(xml): PaperCandidate[]` using `DOMParser` to map each `<entry>` to a `PaperCandidate` with `citationCount`/`references` left `undefined` and `sourceId` built as `` `arxiv:${stripArxivVersion(rawId)}` `` (T003's shared helper) from the Atom `<id>` element, in `src/collection/arxivParser.ts` (per `data-model.md` § Provider response intermediate shapes and `research.md` Decision 13) (FR-007, FR-015, FR-019; depends on T003)
