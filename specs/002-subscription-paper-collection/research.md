...
## 23. `alreadyPersisted`/`persist` depend on a 003 read/query capability that does not yet exist in 003's spec

**Decision**: This feature's contract continues to define `alreadyPersisted(sourceId): Promise<boolean>` and `persist(paper, summary?): Promise<void>` as hooks 002 calls, but explicitly documents — here and in the contract itself — that **`specs-input/003-paper-note-persistence/spec.md` currently defines no query/read capability at all**: its Functional Requirements (FR-001 through FR-014) are exclusively create/update/delete. There is no FR003 promises today that answers "does a record with this `sourceId` already exist," which `alreadyPersisted` needs, or that a caller can use to implement an upsert, which `persist` needs. This feature does not invent or stand in for that capability — it is recorded as an explicit, named dependency on 003's eventual specification, not silently assumed away.

**Rationale**: This is not unique to 002 — at least two other features have the identical, currently-unaddressed dependency: 005's bulk refresh mode needs to enumerate "every saved paper whose `publicationYear` is within the last year," and 006 (graph-data conversion) needs to read back every persisted record to build graph nodes/edges. All three consumers are currently building on an assumption 003's spec doesn't yet promise anything about (003's own Open Questions OQ-6/OQ-8/OQ-9 — in-memory index vs. on-demand reads, load-time scan cost, per-paper files vs. a single index — are exactly the unresolved questions whose answers would determine how such a read/query capability could even be implemented). Recording this explicitly here means that when 003 is actually specified, its author has a concrete, cross-feature-sourced requirement to design against (an existence-check by `sourceId`, and some form of "read back all records," at minimum) rather than discovering the need after 002/005/006 are already built against an unstated assumption.
