...
// recordChecked/onSubscriptionChecked and never influencing lastCheckedAt.
//
// Takes the FRONTIER window (already computed by computeCollectionWindow for THIS check) as
// its anchor, NOT a fresh read of subscription.lastCheckedAt — this is load-bearing, not a
// style choice. By the time a caller might compute the lag window, `onSubscriptionChecked` may
// already have advanced `lastCheckedAt` via `recordChecked` (contracts § subscriptionStore.ts
// call this "immediately after ... onSubscriptionChecked has been called for it" — scheduler.ts
// § Announcement-lag re-scan), so re-reading `subscription.lastCheckedAt` at that point is
// ambiguous: a caller holding a stale `Subscription` object gets the pre-check value (correct),
// but a caller that re-fetches via `getSubscriptions()` (a pattern this very contract elsewhere
// warns returns fresh objects, § in-flight guard) gets the POST-check value — silently anchoring
// the re-scan to `now` instead of the old frontier, and permanently losing exactly the span
// FR-041 exists to catch (worst case: after a long off-period, the entire pre-catch-up lag span).
// Passing the already-computed frontierWindow removes the ambiguity structurally: it is a plain
// number captured once, before any mutation, so there is no "which subscription snapshot" question.
//
// Undefined when there is nothing meaningful to re-scan: a subscription's first check
// (lastCheckedAt === null — no already-checked span exists yet) or a clock-backward reading
// (lastCheckedAt > now — the frontier window is already empty). Otherwise:
//   { from: Math.max(frontierWindow.from - ANNOUNCEMENT_LAG_MS, coveredFrom ?? -Infinity), to: frontierWindow.from }
// `to` is anchored to `frontierWindow.from` — the exact `from` this check's own frontier query
// used — never a value read again afterward. The Math.max(..., coveredFrom ?? -Infinity) clamp
// keeps the re-scan from crossing below the subscription's own backward floor into territory
// only backfill (or nothing yet) has covered.
...
- **Announcement-lag re-scan** (FR-041): the scheduler computes **both** `frontierWindow = computeCollectionWindow(subscription, now)` and `lagWindow = computeLagOverlapWindow(frontierWindow, subscription, now)` **together, upfront, from the same pre-check subscription snapshot** — before invoking `runCheck` for either — so `lagWindow.to` is permanently anchored to the `frontierWindow.from` actually used for this check, never a value re-read after `recordChecked` may have advanced `lastCheckedAt` (see `computeLagOverlapWindow`'s own doc comment for why this ordering is load-bearing, not stylistic). The frontier `runCheck(subscription, frontierWindow)` runs first; only once it resolves successfully (regardless of whether it was `truncated`) and `onSubscriptionChecked` has been called for it does the scheduler, **if `lagWindow` is defined**, invoke `runCheck` a **second time** with the *already-computed* `lagWindow` — using the exact same `runCheck` dependency (no second function needed — only the window differs, exactly like backfill reusing `runCheck` with its own window). This second call's result — `{truncated, coveredThrough}` — is **entirely discarded**: `onSubscriptionChecked` is not called for it, and neither `lastCheckedAt` nor `coveredFrom` are touched. A rejected or truncated lag re-scan is swallowed without surfacing a notice and without affecting the frontier's already-recorded success — since the lag re-scan carries no persisted cursor, it is simply recomputed (from a fresh `frontierWindow`/`lagWindow` pair) and retried on the next check; there is no stalled state to escape from and thus no FR-042-style bound needed here. Both calls run under the *same* per-subscription in-flight-guard duration — the guard is not released between the frontier and lag-re-scan calls — so a `checkNow`/scheduled-tick race against this subscription still cannot start until both queries for this check have completed. If the frontier `runCheck` itself rejects or is skipped (in-flight guard, disabled subscription), the lag re-scan is not attempted this cycle either — it is tied to a successful frontier result. The lag re-scan does not apply when `computeLagOverlapWindow` returns `undefined` (first check or clock-backward — see its own doc comment).
...
export interface PipelineHooks {
  ...
  // Optional: when a paper this subscription collected is ALREADY persisted (earlier run
  // or another subscription), record this subscription's key onto the stored paper's
  // collectedVia set without re-processing it (the lightweight provenance-union path).
...
export function runCollectionPass(
  ...
  subscriptionKey: string, // the collecting subscription's identity (subscriptionKey(sub)); stamped onto new papers' collectedVia and unioned onto already-persisted ones
  hooks: PipelineHooks,
...
// node) and runs them through runCollectionPass, passing subscriptionKey(subscription)
// as the pass's collecting-subscription identity (the Pick it receives is exactly what
// subscriptionKey() consumes). This is the concrete function startScheduler's `runCheck`
...
- The Phase 1 dedup gate is `claim(state, sourceId)`, which classifies each candidate as `'new'`, `'duplicate'` (already `seen` this run), or `'alreadyPersisted'` (`hooks.alreadyPersisted` resolves `true`). A `'duplicate'` or `'alreadyPersisted'` candidate is dropped before it is ever enriched or summarized (FR-009) — no wasted batch-enrichment slot on a known duplicate. For an `'alreadyPersisted'` candidate the pass additionally calls `hooks.recordCollectedVia(sourceId, subscriptionKey)` so the paper records **this** subscription too in its `collectedVia` set (idempotent; the sourceId is marked `seen` so it is unioned at most once per run). A `'new'` paper is stamped `collectedVia = [subscriptionKey]` in Phase 2 immediately after `promote`.
