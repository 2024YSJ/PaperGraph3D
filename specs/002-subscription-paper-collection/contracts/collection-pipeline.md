...
- `cancelBackfill(subscription)` (FR-043): a no-op if `backfillState` is not set. Otherwise it sets `coveredFrom = backfillState.cursor` (crediting the span actually collected — this is the one place besides a natural completion that `coveredFrom` moves) and clears `backfillState` to `null`. Unlike `setEnabled(false)`, it does not disable the subscription and does not affect `lastCheckedAt` — forward collection is entirely unaffected. Identifies target by `(type, value)`.
...
// Three cases, evaluated in this order (FR-024, FR-030, FR-041):
//   1. Clock backward (lastCheckedAt !== null && lastCheckedAt > now): { from: now, to: now }.
//      An empty window — nothing else below applies, the lag overlap never overrides this.
//   2. First check (lastCheckedAt === null): { from: now - 24h, to: now }. FR-030's bound
//      alone, with NO announcement-lag widening — there is no already-checked span yet to
//      widen into, and widening here would pull in pre-registration history, defeating
//      FR-030's flood-prevention purpose.
//   3. Ordinary check: { from: Math.max(Math.min(lastCheckedAt, now - ANNOUNCEMENT_LAG_MS),
//      subscription.coveredFrom ?? -Infinity), to: now }. The FR-041 trailing overlap widens
//      the query window up to ANNOUNCEMENT_LAG_MS earlier than lastCheckedAt (catching arXiv
//      papers not yet announced when an earlier check ran), clamped so it never crosses below
//      the subscription's own coveredFrom backward floor — the overlap augments an
//      ALREADY-CHECKED span, it must never dip into territory only backfill (or nothing yet)
//      has covered.
// This widens the QUERY window only — it never changes what lastCheckedAt is recorded as
// (still the moment actually searched through, FR-006); papers re-fetched in the overlap are
// deduplicated (FR-009) like any overlap duplicate.
...
- On the plugin's first load after `startScheduler` runs, it performs exactly one catch-up pass: for every subscription where `enabled === true`, `runCheck` is invoked once with `window = computeCollectionWindow(subscription, now)` — the three-case computation above (clock-backward empty window / first-check 24h-only / ordinary check's announcement-lag-widened-but-`coveredFrom`-clamped window), `window.to = now` in every case (research.md Decisions 29, 38) (FR-004, FR-024, FR-030, FR-041, Clarification 2026-07-05).
...
- **First-coverage initialization** (FR-035): when a subscription's *first* forward check (its `coveredFrom` still unset) **successfully settles** — at the same point `onSubscriptionChecked` is called, never before `runCheck` resolves — the scheduler calls `deps.onFirstCoverage?.(subscription, window.from)` with **that successful check's** `window.from`, recording `coveredFrom = window.from`. Recording only on success matters: a failed first check leaves `lastCheckedAt` null, so its retry computes a *later* `window.from` (`now − 24h` from the retry moment); pinning `coveredFrom` to the failed attempt's earlier `from` would strand the span between the two — a backfill can't rise above `coveredFrom` and a forward retry starts below the failed `from` — so `coveredFrom` must track the window actually covered, mirroring FR-006's "never record past a window that failed." Idempotent at the store layer (a no-op once set), so calling it on every successful check is harmless. It never affects `lastCheckedAt`.
