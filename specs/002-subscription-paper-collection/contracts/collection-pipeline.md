...
// from = Math.min(lastCheckedAt ?? now - 24h, now - ANNOUNCEMENT_LAG_MS, now). The
// now - ANNOUNCEMENT_LAG_MS term is an unconditional trailing overlap (FR-041): arXiv's
// announcement delay means a paper's submittedDate can precede its actual searchability
// by several days, so every query window reaches back at least ANNOUNCEMENT_LAG_MS no
// matter how recent lastCheckedAt is, catching papers that existed but weren't yet
// announced during an earlier check. This widens the QUERY window only — it never changes
// what lastCheckedAt is recorded as (still the moment actually searched through, FR-006);
// papers re-fetched in the overlap are deduplicated (FR-009) like any overlap duplicate.
...
- On the plugin's first load after `startScheduler` runs, it performs exactly one catch-up pass: for every subscription where `enabled === true`, `runCheck` is invoked once with `window = computeCollectionWindow(subscription, now)` (i.e. `window.from = Math.min(subscription.lastCheckedAt ?? (now - 24 * 3_600_000), now - ANNOUNCEMENT_LAG_MS, now)`, `window.to = now` — the `now - ANNOUNCEMENT_LAG_MS` term is the FR-041 announcement-lag overlap, always applied regardless of how recent `lastCheckedAt` is; the whole expression is clamped so `from` never exceeds `to`, collapsing to an empty window if the clock has moved backward past `lastCheckedAt`; research.md Decisions 29, 38) (FR-004, FR-024, FR-041, Clarification 2026-07-05).
