...
### Session 2026-07-26 (graph-display decisions)

- Q: What controls does the "graph display" settings section contain? → A: **Only the render window.** Node color is a FIXED rule owned by 007 (red = no SPECTER2 embedding, orange = uncited, blue = otherwise), not user-selectable, and the layout is fixed (007's PCA x,y + day z-axis). So the `colorScheme` and `layout` fields are dropped from `GraphDisplayOptions`; the graph-display section has no color or layout control — just the render window.
- Q: What is the render window's default? → A: A **rolling window defaulting to the last 1 year**, with an **"all papers"** option available. (007 FR-021.)
- Q: Does the graph surface any read/unread or delete controls? → A: **No.** Read/unread is dropped entirely (007 OQ-6). "Remove from graph" (007's right-click) is a **session-only hide** and performs no deletion, so the settings screen and the right-click menu carry no read-state or delete control. 007's right-click menu is therefore **five** actions, not six.
...
- Each settings section renders fields owned by the corresponding feature. The current model:
  ...
  - **Graph display options**: **only** the persisted **render window** (007 FR-021) — a rolling window defaulting to the last 1 year, with an "all papers" option — that bounds which papers the graph loads by default. There is no layout or color-scheme control: node color is a fixed rule and the layout is fixed, both owned by 007 (see the 2026-07-26 clarification).
