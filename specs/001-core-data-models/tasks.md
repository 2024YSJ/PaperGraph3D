...
- **Post-completion FR-019 extension (2026-07-07)**: `Paper`/`PaperCandidate` further gained core content-embedding fields — `embedding` (an L2-normalized title+abstract vector, `number[] | null`), `embeddingModel`, and `embeddingSource` (`'local' | 'llm' | null`) — to support the content-similarity graph layout (features `260702-006`/`007`). Added under the open-baseline rule (FR-019/FR-020/FR-021) without a new 001 task; the vector is never a hold-back trigger (a pending `null` is valid) and is populated by `260702-002` (local baseline) / `260702-004` (optional LLM), not by 001. **Follow-up:** `src/models/paper.ts` must add these fields to `Paper`/`PaperCandidate`, carry them through `toPaper()` (defaulting `undefined` → `null`), and extend `isValidPaper()`; the `quickstart.md` samples already carry the new fields. See `data-model.md` § Content embedding and `contracts/data-model-api.md`.
