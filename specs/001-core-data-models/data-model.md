...
| `collectedVia` | `string[]` | Yes (field present; may be empty `[]`) | **Additive extension (collection provenance)**. The de-duplicated set of `subscriptionKey()` strings (`subscription.ts`) of the subscription(s) that collected this paper — a paper may match several subscriptions, so this is a growing **set**, not a single value. Populated by `260702-002`'s pipeline (stamped at promotion; unioned when a later subscription re-collects an already-persisted paper — see `contracts/collection-pipeline.md`). `[]` on a record predating this field (defaulted by `migrate()`) or a paper not produced by subscription collection. Never a hold-back trigger. Not carried on a `PaperCandidate` — the candidate/promotion layer is subscription-agnostic; the pipeline stamps it. |
| `embedding` | `number[] \| null` | Yes (field present; `null` = pending) | **FR-019 (core, 2026-07-07)**. L2-normalized content vector over title + abstract. `null` when not yet computed; never a hold-back trigger (FR-021). Populated by `260702-002` (local baseline / on-device transformer). |
