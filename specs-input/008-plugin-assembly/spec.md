...
4. **Given** any credential field (the Semantic Scholar API key, or the LLM summarization/embedding API key), **When** it is rendered, **Then** it is a masked input that never shows or logs the stored value in plaintext and carries the owning feature's disclosure copy; **and** no arXiv key field is presented, since arXiv needs none.
...
- **Settings Screen**: The single configuration surface with four sections, each delegating to the owning feature's behavior. Sensitive credential fields (Semantic Scholar / LLM API keys) are masked and carry their owning feature's disclosure copy (FR-010); no field is presented for arXiv, which needs no credential.
...
- Each settings section renders fields owned by the corresponding feature — including the summarization provider and credentials, which are 004's extension fields (001 FR-016), and the optional Semantic Scholar API key, which is 002's extension field (002 FR-020). The summarization section also surfaces the **embedding-provider selector** (the explicit three-way choice of 001 FR-022: bundled baseline / user-supplied local transformer / LLM API), the **local-transformer model path or URL** (a 002 extension field, 002 FR-046/FR-021), and the **LLM embedding API key** (a 004 extension field, 004 FR-010); this feature only places these controls. The local-model URL-fetch and LLM disclosure copy (per constitution Principle IV) are owned by 002 and 004 respectively. This feature only surfaces them; their meaning and validation belong to the owning feature. The Semantic Scholar API key is a collection-level credential (not a per-subscription field), so it is surfaced within the subscriptions/collection section; like 004's credentials, it is sensitive and its disclosure copy (what it is sent to, that it is optional, that collection works fully without it) is owned by 002 per constitution Principle IV, this feature only places the field.
- The concrete controls each section places (all owned by, and delegating to, the named feature — this feature adds no logic of its own, FR-005):
  ...
  - **Summarization**: the summarization **enabled** toggle (001 default off, opt-in), the **embedding-provider selector**, the **local-transformer model path/URL**, and the **LLM summarization/embedding API key** (masked; FR-010), owned by 004/002.
  - **Graph display options**: layout and color-scheme controls owned by 007.
