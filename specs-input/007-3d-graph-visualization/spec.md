...
- **OQ-5 — Concrete scale/responsiveness targets. [RESOLVED 2026-07-13]** "Large" is set at **~50,000 nodes in view**, and the responsiveness budget is an **interactive ~50–60 fps** for rotate/zoom/pan on a typical desktop GPU (SC-008). The corpus itself grows without bound — a broad keyword subscription (e.g. bare `transformer`) can add **~100 papers/day (~30k/year)** — so the target is met by two independent levers, not by raw draw speed alone: (1) **rendering/interaction technique** — GPU-instanced node rendering, label/edge level-of-detail, and spatial-index/GPU picking (FR-016–FR-019), which carry tens of thousands of nodes; and (2) **in-view scoping** (FR-020) — a persisted render-window setting (FR-021), the year-range filter, plus optional subscription / similarity-cluster filters — so the whole corpus is never rendered at once and visual clutter stays bounded regardless of total size. Projection cost is separately bounded by 006's cached-basis / out-of-sample policy (006 FR-010). **Not** covered here: bounding *total* corpus accumulation (a retention/cap policy) — deferred to 002/008.
