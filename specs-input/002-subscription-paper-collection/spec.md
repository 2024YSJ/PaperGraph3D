...
- **FR-019**: For every promoted paper, the per-paper pipeline (FR-017) MUST compute a local baseline content embedding (001 FR-019) before persistence, independent of whether summarization or any LLM feature (004) is enabled. Baseline embedding runs offline and requires no credentials, so the similarity layout (006/007) is available even fully offline.
- **FR-020**: When an LLM embedding provider is configured (004), the pipeline MAY replace the baseline embedding with an LLM embedding at the same seam and set `embeddingSource = llm`. An embedding failure (local or LLM) MUST NOT hold the paper back — publication year is the only hold-back trigger — and persistence proceeds with the paper's current-best or pending embedding. Changing the embedding provider MUST trigger re-embedding of the affected persisted papers to preserve space homogeneity (001 FR-020), targeting the persisted store directly rather than rescanning the collection window.
