...
- Q: How does a raw provider response become a valid Paper, and what happens to citation data that a provider such as arXiv does not supply at all? → A: A response is first parsed into a `PaperCandidate` (001), which may carry an *unknown* citation count and unknown references — arXiv returns no citation data, so those stay unknown rather than being set to 0/empty. The candidate is then *promoted* to a valid `Paper` only when it has a publication year (otherwise it is held back — see FR-011). On promotion, an unknown citation count defaults to 0 and unknown references default to an empty list, but the `citationsKnown` flag (see the later 2026-07-05 clarification and FR-018) records that the 0 was unconfirmed, so "unknown" is no longer silently collapsed into "zero". A subscription whose papers need an accurate citation count or relationships for later features (future-directions text 004, uncited-node styling 007) SHOULD still be enriched from a citation-aware provider (Semantic Scholar) *before* promotion so a stored 0 reads "confirmed uncited"; a paper promoted without enrichment is stored reading "0 citations" with `citationsKnown` = false, which a later manual refresh (005) can correct.
...
- Q: Under windowed collection, if enrichment must precede promotion and Semantic Scholar cannot supply a paper's citation data (down, rate-limited, paper not indexed, or identity match fails), holding the paper back drops it from the search window and loses it permanently on the next pass. How is this avoided while keeping windowed collection? → A: Enrichment failure MUST NOT hold a paper back. The ONLY hold-back trigger is a missing publication year (001). A candidate with a real year but unknown citation data is promoted and persisted immediately with a citation count of 0, but the Paper carries a `citationsKnown` flag (added to 001) set to false, meaning "this 0 is unconfirmed, not a real zero". Because the paper is stored right away it never falls out of the windowed search, so the last-checked time advances normally and nothing is lost. The persisted store itself becomes the pending-enrichment queue: refresh (005) and an optional bounded auto-heal re-enrich exactly the `citationsKnown === false` papers, never rescanning the whole window. Enrichment before promotion is still preferred when a confirmed count is needed up front (so a stored 0 reads "confirmed uncited" = `citationsKnown` true); when it cannot be obtained, the flag preserves the unknown-vs-zero distinction that promotion would otherwise collapse. To keep re-enrichment from looping forever, a provider response that positively reports "no such paper" (or a failed identity match) is treated as a terminal absence rather than a transient failure, and transient re-tries are bounded before falling back to manual refresh.

### Session 2026-07-07

- Q: Who computes the mandatory content-embedding baseline, and when in the pipeline? → A: This feature's per-paper pipeline (FR-017) computes the **local baseline embedding at promotion**, before handing the paper to 003 — as a mandatory step that does **not** depend on the summarization/LLM feature (004) being enabled. This keeps the similarity layout (006/007) available fully offline.
- Q: How does the optional LLM-provider embedding fit, given windowed collection must never lose a paper? → A: When an LLM embedding provider is configured (through 004), the pipeline MAY upgrade the baseline to an LLM embedding at the 004 seam. Embedding — baseline or upgrade — failing MUST NOT hold the paper back or delay persistence; the paper is persisted with whatever embedding it has (or pending), exactly like enrichment failure (FR-016/FR-018). A provider change re-embeds affected papers from the persisted store, never by rescanning the collection window.

### Session 2026-07-09

- Q: If an LLM embedding provider is configured, is the local baseline still computed, and which vector becomes the one the graph layout uses? → A: The local baseline is **always** computed at promotion as an immediate fallback (it is bundled, needs no credentials, and never blocks persistence), but it is not necessarily the vector the layout uses. The corpus has exactly one **canonical embedding space** (001 FR-022): when an LLM provider is configured, the LLM `embeddingModel` is canonical, so the pipeline upgrades the baseline to an LLM vector at the 004 seam and the LLM vector is what 006 projects. A paper for which the LLM upgrade has not (yet) succeeded keeps its local baseline vector and is marked pending upgrade — it is re-embedded to the canonical LLM space later from the persisted store (never by rescanning the window), and until then 006 gives it a fallback position rather than mixing it into the LLM projection. When no LLM provider is configured, the local baseline model is itself the canonical space. Because collection is online regardless (it calls arXiv), a user who wants LLM embeddings supplies only an API key — no local model is installed or registered (001 FR-022).

### Session 2026-07-09 (continued)

- Q: The offline embedding option is now a **user-supplied transformer model** (given by a local file path or a URL), so there are three providers (bundled baseline, local transformer, LLM). How is the canonical one chosen, and where does the LLM fit? → A: The canonical provider is an **explicit single selector** (001 FR-022): bundled baseline (default), local transformer, or LLM — exactly one canonical at a time, never implicit precedence. The bundled baseline is always computed as the fallback. For the **local-transformer** option this feature runs the user's model **on-device** via the plugin's bundled inference runtime (FR-021): no data leaves the vault, no credentials. For the **LLM** option it upgrades at the 004 seam (title+abstract only). Both sit at the same "upgrade the baseline" seam and follow the same pending-upgrade / re-embed rules (FR-020); the differences are only where inference runs and whether credentials are needed. A local transformer's vector dimension varies by model, so its `embeddingModel` records the dimension and selecting a different local model is a provider change that re-embeds.
...
- **FR-019**: For every promoted paper, the per-paper pipeline (FR-017) MUST compute a local baseline content embedding (001 FR-019) before persistence, independent of whether summarization or any LLM feature (004) is enabled. The local baseline model MUST be bundled and require no user installation/registration or credentials (001 FR-022); baseline embedding runs offline, so once papers are collected the similarity layout (006/007) renders even fully offline. The baseline vector serves as the always-present fallback so no paper is ever persisted without a vector; whether it is also the vector the layout projects depends on the corpus's canonical space (FR-020).
- **FR-020**: The canonical embedding provider MUST be chosen by an **explicit single selector** (001 FR-022) with three options: the bundled local baseline (default), a user-supplied local transformer (FR-021), or a configured LLM API (004). Exactly one is canonical at a time; selection is explicit, never implicit precedence. The bundled baseline (FR-019) is always computed as the fallback. When the selected provider is not the bundled baseline, the pipeline MUST upgrade that baseline to the selected provider's embedding at the same seam as summarization, setting `embeddingSource` accordingly (`local` for a local transformer, `llm` for the LLM API). An embedding failure (baseline, local transformer, or LLM) MUST NOT hold the paper back — publication year is the only hold-back trigger — and persistence proceeds with the paper's current-best or pending embedding, marking it pending upgrade when the selected canonical vector was not obtained. Only canonical-space vectors are projected together (001 FR-020); off-canonical / pending papers get a deterministic fallback position (006) until re-embedded. Changing the selected provider — including switching to a different local transformer model — MUST trigger re-embedding of the affected persisted papers to the new canonical space to preserve space homogeneity (001 FR-020/FR-022), targeting the persisted store directly rather than rescanning the collection window.
- **FR-021**: When the user selects the **local-transformer** embedding provider (FR-020), they MUST supply the model as a **local file path or a URL**; this feature loads and runs that transformer model **on-device** via the plugin's bundled inference runtime (desktop-only, 007), sending no paper data to any external service and requiring no credentials. A URL source MAY be fetched once and cached locally (a user-initiated fetch of a user-supplied address, not a silent background download); a local-path source is fully offline. The model's `embeddingModel` (identifier + vector dimension, which varies by model) MUST be recorded on each paper it embeds (001 FR-020), so selecting a different local model is a provider change that triggers re-embedding (FR-020). Unlike the LLM path (a 004-owned hook), the local-transformer inference is performed by this feature itself. A local-transformer embedding failure MUST NOT hold a paper back or block persistence — the bundled baseline fallback applies and the paper is marked pending upgrade (FR-019), exactly as an LLM embedding failure is handled.
