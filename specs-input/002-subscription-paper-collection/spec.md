...
- **FR-020**: The corpus has exactly one **canonical embedding space** (001 FR-022): the bundled local model by default, or a configured LLM `embeddingModel`. When an LLM embedding provider is configured (004), the LLM space is canonical and the pipeline MUST upgrade the baseline to an LLM embedding at the same seam and set `embeddingSource = llm`; a user enables this by supplying only an API key. An embedding failure (local or LLM) MUST NOT hold the paper back — publication year is the only hold-back trigger — and persistence proceeds with the paper's current-best or pending embedding, marking it pending upgrade when the canonical LLM vector was not obtained. Only canonical-space vectors are projected together (001 FR-020); off-canonical / pending papers get a deterministic fallback position (006) until re-embedded. Changing the embedding provider MUST trigger re-embedding of the affected persisted papers to the new canonical space to preserve space homogeneity (001 FR-020/FR-022), targeting the persisted store directly rather than rescanning the collection window.
