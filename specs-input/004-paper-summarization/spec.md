...
- **FR-010**: When an embedding-capable LLM provider and credentials are configured, this feature MUST make the configured LLM `embeddingModel` the corpus's canonical embedding space (001 FR-022) and generate an LLM content embedding to replace the paper's local baseline embedding (001 FR-019/FR-020), setting `embeddingSource = llm`; enabling this requires only an API key, never a local-model install (001 FR-022). This is opt-in and strictly additive to the offline path: with it off, the bundled local baseline is the canonical space and the similarity layout (006/007) still works. The local baseline is always retained as a fallback, and 006 projects only the canonical space's vectors.
