...
- **FR-020**: The Paper data definition MUST include `embeddingModel` (a stable model identifier + version) and `embeddingSource` (`local` when produced on-device — by either the bundled baseline model or a user-supplied local transformer, distinguished by `embeddingModel` — `llm` when produced by a configured LLM provider). Embeddings MUST only be compared or jointly projected among papers sharing the one **canonical** `embeddingModel` space (FR-022).
...
- **FR-022**: The canonical embedding provider MUST be chosen by an **explicit single selector** with three mutually exclusive options: (1) the **bundled local baseline** (the default, requiring no setup); (2) a **user-supplied local transformer model**, given as a local file path or a URL and run on-device via the plugin's bundled inference runtime (no credentials, no data leaving the vault); or (3) a **configured LLM API** (requiring only an API key). At any moment exactly one selected provider's `embeddingModel` is the corpus's **canonical embedding space**; selection is explicit, never implicit precedence among whatever happens to be configured. The bundled local baseline model MUST be bundled with the plugin and MUST NOT require the user to install or register anything, and it MUST ALWAYS be computed as an immediate fallback at promotion so no paper is ever persisted without a vector, regardless of which provider is selected. When the selected canonical provider is not the bundled baseline, a paper carrying only its bundled-baseline vector MUST be marked pending upgrade and re-embedded to the canonical space. Only canonical-space vectors may be projected together (FR-020); changing the selected provider — including switching to a different user-supplied local transformer model, whose `embeddingModel` differs — MUST re-embed the corpus from the persisted papers to the new canonical space (owned by 002), never by rescanning collection. Vectors from different providers/models MUST NOT be mixed within a single projection.
...
- **SC-007**: 100% of valid papers carry a content embedding and an accurate `embeddingModel`/`embeddingSource`, so no two papers from different model spaces are ever projected together; only the single canonical space's vectors are projected, and switching the embedding provider re-embeds the corpus rather than mixing spaces (FR-022).
