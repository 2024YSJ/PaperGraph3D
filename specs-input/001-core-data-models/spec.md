...
- **FR-020**: The Paper data definition MUST include `embeddingModel` (a stable model identifier + version) and `embeddingSource` (`local` when produced by the bundled/native baseline model, `llm` when produced by a configured LLM provider). Embeddings MUST only be compared or jointly projected among papers sharing the one **canonical** `embeddingModel` space (FR-022).
...
- **FR-022**: The local baseline model MUST be **bundled** with the plugin and MUST NOT require the user to install or register a model; enabling LLM-provider embeddings MUST require only supplying an API key (collection is online regardless, so no local-model setup is imposed on a user who prefers the LLM path). At any moment the corpus MUST have exactly one **canonical embedding space** — the configured provider's `embeddingModel` (the bundled local model by default, or the configured LLM model). A local baseline vector MUST always be attached at promotion as an immediate fallback so no paper is ever persisted without a vector; when an LLM space is canonical, a paper still carrying only its local vector MUST be marked pending upgrade and re-embedded. Only canonical-space vectors may be projected together (FR-020); changing the embedding provider MUST re-embed the corpus from the persisted papers to the new canonical space (owned by 002), never by rescanning collection. Local and LLM vectors MUST NOT be mixed within a single projection.
